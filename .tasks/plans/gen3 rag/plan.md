...
This is the same mathematical family as Gaussian random fields and harmonic functions on a graph, except each “label” is a (d)-vector.
It is also graph filtering in graph signal processing language.
...
fixing concrete defaults for those five points and adding a more formal probabilistic interpretation where (x) is the MAP estimate of a Gaussian Markov random field