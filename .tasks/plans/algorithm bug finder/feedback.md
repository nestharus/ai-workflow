...
Detects constraint violations in distributed algorithms by analyzing the decorated graph from the [Algorithm Graph Creator](../algorithm%20graph%20creator/feedback.md). This system finds bugs through **invariant collision detection**—identifying mismatches between what components expect and what they receive.

This is a **verification tool**, not an enhancement tool. It answers: "Is the algorithm internally consistent with its declared constraints?"
...
> **VIOLATION:** COM-14 expects `INV-DETERMINISTIC` from COM-05 via CON-05, but COM-05 does not guarantee it.
...
> **VIOLATION:** `OBL-SERIAL-ACCESS` from COM-99 (which has invariant INV-STORES-FILE-REGISTRY) leaked to root COM-14. No component in the path provides serialization.

---

### Type 3: Context/Constraint Collision

An invariant from the execution context conflicts with a constraint from a component (often one with state storage invariants).

**ContextCollision**
- The context invariant (top-down, e.g., SET-PARALLEL)
- The constraint (bottom-up, e.g., OBL-SERIAL)
- Where the collision occurs
- Sources of both the context and constraint
...
flowchart LR
    ...
    B -->|Yes| collision
    B -->|No| C([Continue]):::continue
    subgraph collision[Collision Handling]
        D[Create ContextCollision]:::error
    end
    D --> C
...
flowchart TB
    ...
    classDef context fill:#bfdbfe,stroke:#1e40af,color:#1e3a8a
    ...
    classDef constraint fill:#fee2e2,stroke:#b91c1c,color:#7f1d1d
...
> **COLLISION at COM-05:**
> - **Context:** SET-05 (Parallel) from COM-14
> - **Constraint:** OBL-20 (Serial) from COM-99 (which has state invariant INV-STORES-REGISTRY)
> - These are incompatible—COM-05 must implement synchronization.

---

## The Constraint Collision Prompt

When a collision is detected, generate a focused prompt for LLM analysis:

| | |
|---|---|
| **Section 1: 🌐 Operating Environment**<br>Component X called by Y<br>Active Context: invariant | **Section 2: 📦 Deep State Boundary**<br>Component X touches Z<br>Active Constraint: obligation |
| **Section 3: ⚡ The Conflict**<br>Running in context A<br>but touching resource requiring B | **Section 4: ❓ Analysis Required**<br>Real conflict or false positive?<br>What mechanism needed?<br>Where to implement? |
...
flowchart LR
    subgraph Concurrency["🔄 Concurrency Conflicts"]
        direction LR
        PAR["SET-PARALLEL-EXEC"]:::context -.->|conflicts| SER["OBL-SERIAL-ACCESS"]:::constraint
        PAR -.->|conflicts| SW["OBL-SINGLE-WRITER"]:::constraint
        ASYNC["SET-ASYNC-CONTEXT"]:::context -.->|conflicts| SYNC["OBL-SYNC-ONLY"]:::constraint
        ASYNC -.->|conflicts| BLOCK["OBL-BLOCKING-ALLOWED"]:::constraint
    end

    subgraph Ordering["📋 Ordering Conflicts"]
        direction LR
        UNORD["INV-UNORDERED-OUTPUT"]:::context -.->|conflicts| ORD["OBL-ORDERED-INPUT"]:::constraint
    end

    subgraph State["💾 State Conflicts"]
        direction LR
        STATELESS["SET-STATELESS"]:::context -.->|conflicts| STATEREQ["OBL-STATE-REQUIRED"]:::constraint
    end

    subgraph Resource["🔌 Resource Conflicts"]
        direction LR
        UNBOUNDED["SET-UNBOUNDED-CONCURRENCY"]:::context -.->|conflicts| LIMITED["OBL-LIMITED-CONNECTIONS"]:::constraint
    end
...
**Use Case:** Understanding which components are truly isolated (safe to reason about independently) vs. which are coupled through leaked constraints.
...
flowchart LR
    ...
    subgraph analysis[⚙️ Analysis Methods]
        ...
        CC["find_context_collisions"]:::method
...
Note on "logic bugs": If behavior is incorrect, it violates some expectation—which is an invariant. The system catches this through:
- **Inherited invariants** (from parents) vs **inferred behavior** (from code) mismatches
- Parent says "output must be sorted" → code produces unsorted → violation detected

Note on "missing constraints": The [Graph Creator](../algorithm%20graph%20creator/feedback.md) infers implicit invariants from algorithm content. You don't have to manually declare every invariant—the LLM expands the invariant set by analyzing what the code actually assumes and requires.

The bug finder answers: "Given the constraints (declared + inferred + inherited), is the system internally consistent?"
