...
**ULID Usage in Workflows**:
- `run_id` and `step_execution_id` are ULIDs generated by the runtime
- Workflows MUST NOT assume ordering relationships between ULIDs from different processes
- For ordering, use explicit `depends_on` relationships in workflow steps
- For event correlation, use `run_id` + `step_execution_id` pairs, not ULID comparison
