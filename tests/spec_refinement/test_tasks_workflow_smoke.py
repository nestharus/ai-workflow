...
def test_plan_tasks_validates_coverage(interface_workspace, mock_task_agents, fs) -> None:
    ...
    assert any(call["agent_name"] == "chatgpt-task-plan-judge" for call in controller.call_log)
...
def test_plan_tasks_repair_loop_succeeds(interface_workspace, mock_task_agents, fs) -> None:
    ...
    assert any(call["agent_name"] == "chatgpt-task-plan-repairer" for call in controller.call_log)
...
def test_plan_tasks_repair_loop_fails_after_max_retries(
    ...
    repair_calls = [
        call for call in controller.call_log if call["agent_name"] == "chatgpt-task-plan-repairer"
    ]
    assert len(repair_calls) == 3


def test_plan_tasks_handles_no_gaps_or_decisions(interface_workspace, mock_task_agents, fs) -> None:
...
def test_plan_tasks_uses_architecture_mapping(interface_workspace, mock_task_agents, fs) -> None:
    ...
    prompts = [
        call["prompt"] for call in controller.call_log if call["agent_name"] == "opus-task-planner"
    ]
    assert prompts
...
def test_plan_tasks_includes_decisions_in_tasks(interface_workspace, mock_task_agents, fs) -> None:
...
def test_plan_tasks_renders_markdown_index(interface_workspace, mock_task_agents, fs) -> None:
