...
description: Resolves git merge conflicts by analyzing both sides' intent and stitching changes together
model: Claude Opus 4
tools: ['*']
---
...
Examine what the source branch changed from the base (run in sandbox using
#tool:terminal):
...
Use the **source_path** (original clean worktree) to understand the codebase using
#tool:search and file reading tools:

```bash
# Search for related code patterns using #tool:search
...
Read the file with conflict markers from the **sandbox** using file reading tools:
...
**Important**: Edit the file in the **sandbox_path**, not the source_path using the
Edit tool:
...
* Stage the file in the sandbox after resolution using #tool:terminal: `cd <sandbox_path> && git add <file_path>`
