---
name: Lint Fixer
description: Resolves and fixes lint errors and warnings iteratively until all issues are resolved
tools: ['*']
...
When `--changed-only` is specified, use the following commands (#tool:terminal) to detect changed files:
...
Then pass the files to the lint command (#tool:terminal):
```bash
...
**NEVER run `uv run lint` without arguments (#tool:terminal).** This runs ALL linters at once which is
...
When `--worktree` is specified, prepend `cd <worktree> &&` to all commands (#tool:terminal) or use the
...
If not using `--changed-only`, run (#tool:terminal):
```bash
...
**Standard mode** (no `--changed-only`) - use #tool:terminal:
```bash
...
**Changed-only mode** (with `--changed-only`) - use #tool:terminal:
```bash
...
For each linter: Keep running (#tool:terminal) and fixing until it passes before moving
...
- Allow up to 2 hours for lint command (#tool:terminal); do not stop it early
