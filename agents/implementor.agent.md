...
The plan file contains a single plan section starting with `### Plan N: [Title]` followed by implementation instructions. Use #tool:search to locate files and #tool:fetch to read them.
...
6. Run relevant tests after implementation using #tool:terminal:
...
- Use repo tools/scripts (via #tool:terminal) instead of ad-hoc commands when available.
- Prefer fixing code over modifying tests unless tests are incorrect.
- Use #tool:fetch to search for documentation when stuck on unfamiliar patterns.
