---
description: Run lint-fixer sub-agent until all lint errors and warnings are fixed
name: lint-fix
...
Check if `--changed-only` was provided in the user's message:

* If `--changed-only` is present, set `CHANGED_ONLY_MODE=true`
* Otherwise, set `CHANGED_ONLY_MODE=false`

When in changed-only mode, detect the changed files once at the start for use in Step 3 using #tool:terminal:
...
Invoke the lint-fixer sub-agent with the appropriate arguments using #runSubagent:

**Standard mode** (no `--changed-only`):

Use #runSubagent to delegate to the Lint Fixer agent with an empty prompt.

**Changed-only mode** (with `--changed-only`):

Use #runSubagent to delegate to the Lint Fixer agent with prompt "--changed-only".
...
1. Run to check current lint status using #tool:terminal:
...
1. Run linters with `--files` flag using the changed files from Step 0 using #tool:terminal:
...
Use #runSubagent to delegate to a general-purpose agent to investigate and fix linter conflicts:
...
3. Use web search to research solutions:
...
* **Track progress** - keep track of remaining error counts per iteration
