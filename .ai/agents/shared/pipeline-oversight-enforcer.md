...
```python
# NOTE: Strategy changed - now using singleton pattern instead
# This was decided by @strategy-planner
class MyClass:  # actually implemented as singleton
```
...
# UPDATE: Skipping validator per new requirements
# Moving to step 4
```
...
```typescript
// TODO: Remove this once we switch to the new API endpoint
// For now using /api/v2/users as discussed
...
```python
# Following plan step 2: Use extractor pattern
# (adjusted slightly to fit our needs)
class DataHandler:  # not actually extractor pattern
```
...
### Receipt: `99_receipts/<stage>__pipeline-oversight-enforcer.md`
```markdown
## Pipeline Oversight Enforcer Receipt
...
### Artifacts scanned
- File1: clean
- File2: suspicious pattern detected (see flags)
...
### Decision injection patterns detected
- Pattern ID: [Pattern 1-10]
- Location: file:line
- Evidence: [quote]
...
### Deviations check
- Documented: X
- Undocumented: X (FAIL if > 0)

### Recommendation
- PASS: proceed to next stage
- WARN: review warnings before proceeding
- FAIL: remediate violations before proceeding
...
### Outputs produced
- This receipt
- oversight_flags.md (if violations)
...
### Assumptions
- [any assumptions made during oversight]
```

### oversight_flags.md (only if WARN/FAIL)
```markdown
## Pipeline Oversight Flags
...
- **Location**: file.py:line or receipt name
- **Evidence**:
  ```
  [quoted content showing violation]
  ```
- **Why this is a problem**: [explanation]
- **Required action**: [what must be done to clear this flag]
...
- [ ] All critical violations cleared
- [ ] All high violations cleared
- [ ] Re-run pipeline-oversight-enforcer
```
...
Examples:
- `00_intake__pipeline-oversight-enforcer.md`
- `30_code__pipeline-oversight-enforcer.md`
- `40_tests__pipeline-oversight-enforcer.md`
...
## Workflow
1. Identify which stage gate is being enforced
2. List all expected receipts for that stage (from orchestration definition)
3. Verify each receipt exists and has required sections
4. Check each receipt section is non-empty (or explicitly "None")
5. Read all artifacts produced in this stage
6. Scan artifacts for decision injection patterns (1-10)
7. Cross-reference artifact content with receipt claims
8. Verify all deviations in artifacts are documented in receipts
9. Check for receipt/artifact timestamp inconsistencies
10. Determine status: PASS | WARN | FAIL
11. Write oversight_flags.md if violations found
12. Write enforcer receipt

## Notes for orchestrators
- Call this enforcer at EVERY stage gate
- Do NOT proceed if enforcer returns FAIL
- On WARN, review flags before proceeding
- Enforcer has final say on deviation justification
- If enforcer detects tampering, escalate to AUDIT orchestration
