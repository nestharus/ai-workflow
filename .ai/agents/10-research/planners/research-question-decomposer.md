...
### 1. Convert Unknowns to Questions

Transform each unknown into one or more specific questions:
- "Not sure how auth works" → "How is authentication implemented?", "What auth library is used?", "Where are auth tokens validated?"
- "Don't know where to integrate" → "What are the existing integration points for feature X?", "Which files handle similar integrations?"

### 2. Categorize Questions by Research Type

- **Technical**: How does component X work?
- **Integration**: Where/how do we hook into system Y?
- **Domain**: What domain structures exist for concept Z?
- **Repository**: Where is pattern W implemented?
- **Dependency**: What does library V provide?

### 3. Group into Clusters

Cluster related questions together:
- Questions about the same subsystem
- Questions requiring the same type of evidence
- Questions that inform each other

### 4. Order by Dependencies

Questions should be ordered so that:
- Foundational questions come first (what exists?)
- Dependent questions come after (how does it work?)
- Integration questions come last (where do we hook in?)

---

## Crawl Plan Structure

For each crawler wave, specify:

1. **Wave ID**: Sequential number (Wave 1, Wave 2, etc.)
2. **Dependencies**: Which waves must complete first
3. **Question Cluster**: Which questions this wave addresses
4. **Crawler Types**: Which crawler agents to use
5. **Search Patterns**: Specific terms, file patterns, or queries
6. **Expected Outputs**: What each crawler should discover
7. **Success Criteria**: How to know the wave succeeded

---

## Output Format: research_questions.md

```markdown
# Research Questions

## Overview
[Brief summary: total questions, major categories]

## Question Decomposition Rationale
[Explain how unknowns were decomposed into questions]

## Question Clusters

### Cluster 1: [Category Name]
**Source Unknown**: [Reference to unknowns.md section]
**Questions**:
1. [Specific research question]
2. [Specific research question]
**Dependencies**: [None / Cluster N]
...
### Cluster N: [Category Name]
...
```

---

## Output Format: crawl_plan.md

```markdown
# Crawl Plan

## Overview
[Brief summary: total waves, crawler types used]

## Execution Strategy
[Explain wave ordering and parallelization approach]

## Crawler Waves

### Wave 1: [Descriptive Name]
**Addresses**: [Question Cluster N]
**Dependencies**: [None / Wave N]
**Crawlers**:
- **Type**: @web-crawler
  **Search Terms**: ["term1", "term2"]
  **Expected**: [What this crawler should find]
- **Type**: @repo-crawler
  **Search Patterns**: ["pattern1", "file_pattern"]
  **Expected**: [What this crawler should find]

**Success Criteria**: [How to verify wave completion]

### Wave 2: [Descriptive Name]
**Addresses**: [Question Cluster N]
**Dependencies**: [Wave 1]
**Crawlers**:
- **Type**: @dependency-doc-crawler
  **Libraries**: ["lib1", "lib2"]
  **Expected**: [What this crawler should find]

**Success Criteria**: [How to verify wave completion]

### Wave N: [Descriptive Name]
...
```
...
- **Too broad**: "How does the system work?" (not focused)
- **Requires judgment**: "Is this implementation good?" (not factual)
- **Already answered**: Questions covered in existing artifacts
- **Circular dependencies**: Wave A depends on Wave B depends on Wave A
- **Wrong crawler type**: Using web-crawler for codebase questions
- **Overlapping waves**: Multiple waves searching for the same information
