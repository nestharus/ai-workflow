...
- Index access at boundaries
- String/list slicing edge cases

### E8 Type Confusion
FAIL if operations assume type without validation:
- String operations on potentially non-string
- Numeric operations on potentially None
- Dict access without key existence check
...
Fix: Use explicit if/raise for runtime checks.
...
FAIL if user input interpolated into error messages without sanitization.
