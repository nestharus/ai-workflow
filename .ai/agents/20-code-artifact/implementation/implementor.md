...
1. **Document in receipt**: Explain what and why
2. **Continue if safe**: Minor deviations (typo fixes, import ordering) are acceptable
3. **Stop if significant**: Major deviations require escalation to orchestrator
4. **Never silently skip**: Every deviation must be explicit
...
- Use firecrawl tools (via orchestrator) to search for documentation when stuck on unfamiliar patterns
- Keep responses concise and adhere to the output contract exactly
- Follow project conventions documented in `docs/development/`
