...
The implementation_plan.md must follow this exact structure:
...
## Additional Info
[Any relevant context, constraints, or considerations from research_findings]

## Plans

### Plan 1: [Title]
[Detailed implementation steps - see Plan Step Requirements below]

### Plan 2: [Title]
[If needed - detailed implementation steps]

## Execution Instructions
[Order of execution, dependencies between plans, parallelization opportunities]

## Success Criteria
[List concrete, measurable criteria derived from acceptance_criteria.md]
```
...
For each plan step (### Plan N):

### 1. Structures used
- List patterns/primitives (e.g., Router/Dispatcher, Validator, Middleware, State store, Side-effect boundary)
- Reference pattern_pack.md vocabulary when possible

### 2. Code units created/modified
- Each with a tag from the building blocks taxonomy (Extractor/Transformer/Validator/etc.)
- Include file path and symbol names when possible
- Format: `app/services/foo.py::FooService` (Transformer)

### 3. Integration points
- Where it plugs into repo (from repo_integration_map.md)
- Existing call sites that need updates
- New entry points created

### 4. Acceptance criteria coverage
- Which AC items this step satisfies (reference by number/ID)

### 5. Side-effect boundaries (when applicable)
- External calls (APIs, databases, file I/O, network)
...
Plans MUST adhere to these structural rules:

1. **Sequential numbering**: Plan 1, Plan 2, Plan 3... (no gaps, no letter suffixes like "Plan 2a")
2. **Required sections**: Each plan must have Overview, Current State, Target State, Plans, Success Criteria
3. **Measurable success**: Success criteria must be verifiable (tests pass, coverage met, etc.)
4. **No unauthorized stubs**: Every plan step must be fully implementable or explicitly deferred to a later plan
5. **Independent plans**: Each plan should be independently implementable and reviewable
