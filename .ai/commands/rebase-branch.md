...
argument-hint: "`ticket-id`, `PR-id`, `branch-name`, or empty for current branch"
...
* **Sandbox server not running**: The command will fail if the sandbox server is not running. Start it with `docker compose -p ai-workflow-devtools -f docker-compose.dev.yml up -d`
