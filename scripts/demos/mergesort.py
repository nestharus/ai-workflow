...
def get_alignment_signal(arr):
    if len(arr) <= 1: return np.array([0.0])
    signal = []
...
def scanner_sorter_gen(source_data):
    ...
    for d in range(max_depth):
        ...
        for chunk in parents:
            if not chunk: 
                sig_data.append((None, None, None, None)); child_idx+=2; continue
            mid = len(chunk) // 2
    ...
    for d in range(max_depth, 0, -1):
        ...
        while True:
            ...
            for p_idx in range(num_pairs):
                ...
                winner_side = None
                if cL < len(L) and cR < len(R):
                    if L[cL] <= R[cR]: winner_side = 0
                    else: winner_side = 1
                elif cL < len(L): winner_side = 0
                elif cR < len(R): winner_side = 1
                
                moves.append(winner_side)
            
            if not step_active: break
...
def plot_horz_interpolated(ax, cx, cy, data, camera_y, start_sig, end_sig, progress):
    ...
    if len(data) == 1:
        w = UNIT_H
        ax.plot([cx - w/2, cx + w/2], [cy - camera_y, cy - camera_y], color='red', linewidth=LINE_WIDTH)
        return
    if start_sig is None: start_sig = get_alignment_signal(data)
    if end_sig is None: end_sig = get_alignment_signal(data)
...
def plot_scanner_active(ax, cx, cy, source_data, history_data, cursor_val, is_moving, anim_progress, camera_y):
    """
...
def update(frame_data):
    ...
    for d in range(1, max_d + 1):
        for node in layout[d]:
            ...
            if abs(py) < 800 or abs(cy) < 800:
                ax.plot([p_node['cx'], node['cx']], [py, cy], color='white', linewidth=1.5, zorder=0)
    ...
    if phase == 'divide':
        ...
        for p_idx, p in enumerate(puzzles):
            ...
            plot_scanner_active(ax, l_node['cx'], l_node['cy'], tree_data[d][l_idx], p['L_hist'], c_l, move_side==0, anim_progress, camera_y)
            plot_scanner_active(ax, r_node['cx'], r_node['cy'], tree_data[d][r_idx], p['R_hist'], c_r, move_side==1, anim_progress, camera_y)
        ...
        for p_idx, p in enumerate(puzzles):
            ...
            c_l, c_r = cursors[p_idx] if cursors else (len(tree_data[d][l_idx]), len(tree_data[d][r_idx]))

            plot_scanner_active(ax, cur_lx, cur_ly, tree_data[d][l_idx], p['L_hist'], c_l, False, 0.0, camera_y)
            plot_scanner_active(ax, cur_rx, cur_ry, tree_data[d][r_idx], p['R_hist'], c_r, False, 0.0, camera_y)
        ...
        plot_scanner_active(ax, root['cx'], root['cy'], final, final_hist, len(final), False, 0.0, camera_y)
        ax.set_title("SORTED", color='#00ff00', fontsize=20, y=0.85)

ani = animation.FuncAnimation(fig, update, frames=sorter_gen, blit=False, interval=INTERVAL, repeat=False)
