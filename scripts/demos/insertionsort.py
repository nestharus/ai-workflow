...
def scale_cannon_generator(source_data, speed=1):
    ...
    while left_pool or right_pool:
        ...
        for _ in range(2):
            ...
            full_array = np.concatenate(
                [left_pool if left_pool else [], temp_buffer, right_pool if right_pool else []]
            )
        ...
        full_array = np.concatenate(
            [left_pool if left_pool else [], sorted_buffer, right_pool if right_pool else []]
        )
...
pan_height = 0.8
left_pan = Polygon(
    np.array([[0, 0], [pan_width, 0], [pan_width - 0.5, -pan_height], [0.5, -pan_height]]),
    closed=True,
    facecolor="#b08030",
    edgecolor="#8b6914",
    linewidth=2,
...
left_weight = ax.text(
    0, 0, "", ha="center", va="center", fontsize=7, color="white", fontweight="bold", zorder=18
)
right_weight = ax.text(
    0, 0, "", ha="center", va="center", fontsize=7, color="white", fontweight="bold", zorder=18
)

# Flying element
flying_element = Circle(
    (0, 0), 0.4, facecolor="#ff6b6b", edgecolor="#cc4444", linewidth=2, zorder=25, visible=False
)
...
left_cannon = Polygon(
    cannon_shape_right, closed=True, color="#484f58", ec="#30363d", lw=2, zorder=18
)
right_cannon = Polygon(
    cannon_shape_left, closed=True, color="#484f58", ec="#30363d", lw=2, zorder=18
)
...
def update(frame_data):
    ...
    show_left_ball = left_val is not None and (
        phase not in ["launching", "firing"]
        or (phase in ["launching", "firing"] and tilt_side != "left")
    )
    if show_left_ball:
    ...
    show_right_ball = right_val is not None and (
        phase not in ["launching", "firing"]
        or (phase in ["launching", "firing"] and tilt_side != "right")
    )
    if show_right_ball:
