r"""Orchestrate fact isolation workflow with validation and reporting.
...
Usage:
    ...
    uv run knowledge.isolate-entity-facts \
      --sentence "Mount all versioned endpoints under /api/{version} using \\
        create_app in app/core/factory.py." \
      --entity "create_app"

    # Dry run (show report without storing)
    uv run knowledge.isolate-entity-facts --sentence "..." --entity "FastAPI" \\
        --dry-run
...
def isolate_entity_facts_main(args: argparse.Namespace) -> int:
    ...
    print(
        f"  [{status_info}] Information preserved "
        f"(similarity: {validation['semantic_similarity']:.2f})"
    )
