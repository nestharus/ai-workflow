...
def _format_csv(
    ...
    for m in manifests:
        ...
        if show_contributors:
            ...
            row["structural_count"] = len(contributors.get("structural", []))
            row["semantic_count"] = len(contributors.get("semantic", []))
...
def _compute_stats(manifests: list[ArtifactManifest]) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _logger.error("Failed to load manifests: %s", exc)
        return 1
