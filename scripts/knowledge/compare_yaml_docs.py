...
def _check_sibling_constraints(
    ...
    for constraint in sibling_constraints:
        ...
        if "equals" in constraint and sibling_value != constraint["equals"]:
            return False
        ...
        if "starts_with" in constraint and not str(sibling_value).startswith(
            constraint["starts_with"]
        ):
            return False

        # Check ends_with operator
        if "ends_with" in constraint and not str(sibling_value).endswith(constraint["ends_with"]):
            return False
...
def _check_content_sniff(
    ...
    if not content_sniff or not isinstance(value, str):
        return bool(not content_sniff)

    # Check starts_with_any
    starts_with_any = content_sniff.get("starts_with_any", [])
    if starts_with_any and not any(value.startswith(prefix) for prefix in starts_with_any):
        return False

    # Check ends_with_any
    ends_with_any = content_sniff.get("ends_with_any", [])
    if ends_with_any and not any(value.endswith(suffix) for suffix in ends_with_any):
        return False
    ...
    for kind in registry:
        ...
        if kind.get("kind_id") == artifact_kind:
            ...
            if raw_mode in ("full", "incremental", "query_only"):
                extraction_mode: ExtractionMode = raw_mode
            else:
            ...
            for key, value in node.items():
                ...
                if _is_scalar(value):
                    ...
                    if "$ref" in value and isinstance(value["$ref"], str):
                        ...
                        scope_path, _, _ = child_path.rpartition(".")
                        ref_kind: ValueKind = "ref"
                        role_assignment = _assign_role(key, ref_kind, child_path, node, value)
                        group_key = _compute_group_key(scope_path, child_path, node)
                        group_id = _compute_group_id(group_key)
...
def _collect_containment_edges_from_files(file_paths: list[Path]) -> list[ContainmentEdge]:
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            _logger.debug(f"Could not parse {relative_path}: {e}")
            continue
    return all_edges
