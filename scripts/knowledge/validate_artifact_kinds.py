...
def validate_structure_pattern_determinism(
    ...
    for kind in kinds:
        ...
        if root_path and not re.match(r"^[a-zA-Z_][a-zA-Z0-9_\[\]\*\.]*$", root_path):
            result.add_error(
                kind_id,
            ...
            for idx, constraint in enumerate(sibling_constraints):
                ...
                if not has_operator and "key" in constraint:
                    result.add_error(
                        kind_id,
                        f"structure_pattern.sibling_constraints[{idx}]",
                        "Must specify a matching operator "
                        "(equals, matches, starts_with, ends_with)",
                    )
...
def validate_samples(
    ...
    for kind in kinds:
        ...
        for idx, example in enumerate(examples):
            ...
            _element_data, element_facts, resolve_error = _resolve_element_and_field(
...
def validate_duplicates(
    ...
    for _sig, kind_ids in signatures_to_kinds.items():
        if len(kind_ids) > 1:
            result.add_error(
                kind_ids[0],
                "structure_pattern",
