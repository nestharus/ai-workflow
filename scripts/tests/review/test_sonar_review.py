"""Tests for scripts.dev.review.sonar_review module."""
...
from scripts.dev.review import sonar_review
...
# Path where sonar_review.py looks for sonar_scan.sh
SONAR_REVIEW_DIR = Path(sonar_review.__file__).resolve().parent
...
class TestRunSonar:
    ...
    def test_creates_output_directory(self, fs: FakeFilesystem) -> None:
        ...
        # Use SONAR_REVIEW_DIR defined at module level
        fs.create_file(str(SONAR_REVIEW_DIR / "sonar_scan.sh"), contents="#!/bin/bash\necho test")

        with patch("scripts.dev.review.sonar_review.run_command_with_tee", return_value=0):
    ...
    def test_calls_sonar_script(self, fs: FakeFilesystem) -> None:
        ...
        # Use SONAR_REVIEW_DIR defined at module level
        script_path = SONAR_REVIEW_DIR / "sonar_scan.sh"
        fs.create_file(str(script_path), contents="#!/bin/bash\necho test")
        fs.create_dir("/output")

        with patch("scripts.dev.review.sonar_review.run_command_with_tee") as mock_tee:
    ...
    def test_raises_system_exit_on_failure(self, fs: FakeFilesystem) -> None:
        ...
        with patch("scripts.dev.review.sonar_review.run_command_with_tee", return_value=1):
