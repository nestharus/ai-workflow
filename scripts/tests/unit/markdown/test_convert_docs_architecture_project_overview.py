"""Tests for scripts.dev.markdown.convert_docs_architecture_project_overview module."""
...
class TestMain:
    ...
    def test_main_calls_converter_with_correct_paths(self) -> None:
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_architecture_project_overview.MarkdownConverter"
        ) as mock_converter_class:
            mock_converter = MagicMock()
            mock_converter.run.return_value = mock_doc
            mock_converter_class.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_architecture_project_overview import (
                main,
            )
            ...
            assert "project-overview.md" in call_args[0]
            assert "project-overview.yml" in call_args[1]
