"""Tests for scripts.dev.markdown.convert_docs_testing_testing_workflow module."""
...
class TestMain:
    ...
    def test_main_calls_converter_with_correct_paths(self) -> None:
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_testing_testing_workflow.MarkdownConverter"
        ) as mock_converter_class:
            mock_converter = MagicMock()
            mock_converter.run.return_value = mock_doc
            mock_converter_class.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_testing_testing_workflow import (
                main,
            )
            ...
            assert "testing-workflow.md" in call_args[0]
            assert "testing-workflow.yml" in call_args[1]

            # Verify run() was called
            mock_converter.run.assert_called_once()

    def test_main_prints_output(self, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    def test_main_uses_project_root_paths(self) -> None:
        ...
        with patch(
            ...
            # Paths should contain docs/testing
