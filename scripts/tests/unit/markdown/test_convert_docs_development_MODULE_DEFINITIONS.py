"""Tests for scripts.dev.markdown.convert_docs_development_MODULE_DEFINITIONS module."""
...
class TestMain:
    ...
    def test_main_calls_converter_with_correct_paths(self) -> None:
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_development_MODULE_DEFINITIONS.MarkdownConverter"
        ) as mock_converter_class:
            mock_converter = MagicMock()
            mock_converter.run.return_value = mock_doc
            mock_converter_class.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_development_MODULE_DEFINITIONS import (
                main,
            )
            ...
            assert "MODULE-DEFINITIONS.md" in call_args[0]
            assert "MODULE-DEFINITIONS.yml" in call_args[1]
    ...
    def test_main_uses_project_root_paths(self) -> None:
        ...
        with patch(
            ...
            # Paths should contain docs/development
            assert "docs" in source_path
            assert "development" in source_path
            assert "docs" in target_path
            assert "development" in target_path
