"""Tests for scripts.dev.markdown.convert_docs_plans_phase2 module."""
...
class TestMain:
    ...
    def test_main_calls_converter_with_correct_paths(self) -> None:
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_plans_phase2.MarkdownConverter"
        ) as mock_converter_class:
            mock_converter = MagicMock()
            mock_converter.run.return_value = mock_doc
            mock_converter_class.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_plans_phase2 import (
                main,
            )
            ...
            assert "phase2.md" in call_args[0]
            assert "phase2.yml" in call_args[1]
