...
class TestAssertNonTargetPreservation:
    ...
    def test_raises_when_non_targets_removed(self) -> None:
        ...
        original_text = (
            "Target sentence. Important non-target sentence. "
            "Another non-target sentence. Third non-target."
        )
...
class TestLoadArtifactManifestInvalidFormat:
    """Tests for load_artifact_manifest with invalid manifest format."""

    def test_raises_error_for_non_dict_manifest(self, tmp_path: Path) -> None:
        """Should raise ArtifactExtractionError when manifest is not a dict (line 345)."""
        knowledge_path = tmp_path / ".knowledge"
        artifacts_dir = knowledge_path / "artifacts"
        artifacts_dir.mkdir(parents=True)

        # Create a YAML file that parses to a list instead of a dict
        manifest_path = artifacts_dir / "list-artifact.yml"
        manifest_path.write_text("- item1\n- item2\n- item3", encoding="utf-8")

        with pytest.raises(ArtifactExtractionError, match="Invalid manifest format"):
            load_artifact_manifest("list-artifact", knowledge_path)

    def test_raises_error_for_scalar_manifest(self, tmp_path: Path) -> None:
        """Should raise ArtifactExtractionError when manifest is a scalar value."""
        knowledge_path = tmp_path / ".knowledge"
        artifacts_dir = knowledge_path / "artifacts"
        artifacts_dir.mkdir(parents=True)

        # Create a YAML file that parses to a string
        manifest_path = artifacts_dir / "scalar-artifact.yml"
        manifest_path.write_text("just a simple string", encoding="utf-8")

        with pytest.raises(ArtifactExtractionError, match="Invalid manifest format"):
            load_artifact_manifest("scalar-artifact", knowledge_path)


class TestLoadArtifactTextSourceElementId:
    """Tests for load_artifact_text with source_element_id (lines 385-398)."""
...
class TestExtractArtifactFactsNoOpDetection:
    """Tests for extract_artifact_facts no-op detection (lines 828-831)."""
...
class TestExtractArtifactFactsNoEntities:
    """Tests for extract_artifact_facts when no entities discovered (lines 709-711)."""
...
class TestExtractArtifactFactsInvariantViolationRevert:
    """Tests for extract_artifact_facts invariant violation revert (lines 817-823)."""
...
class TestMainFunction:
    """Tests for main function (lines 978-1025)."""

    def test_main_success(self, tmp_path: Path) -> None:
        """Should return 0 on successful extraction (line 1021)."""
        knowledge_path = tmp_path / ".knowledge"
        (knowledge_path / "facts").mkdir(parents=True)
        (knowledge_path / "facts" / "residue").mkdir(parents=True)
        artifacts_dir = knowledge_path / "artifacts"
        ...
        manifest = {"artifact_id": "main-test", "inline_text": "Test content."}
        manifest_path = artifacts_dir / "main-test.yml"
        manifest_path.write_text(yaml.dump(manifest), encoding="utf-8")
    ...
    def test_main_with_absolute_knowledge_path(self, tmp_path: Path) -> None:
        """Should handle absolute knowledge path (line 988)."""
        knowledge_path = tmp_path / ".knowledge"
        (knowledge_path / "facts").mkdir(parents=True)
        (knowledge_path / "facts" / "residue").mkdir(parents=True)
        artifacts_dir = knowledge_path / "artifacts"
        ...
        manifest = {"artifact_id": "abs-path-test", "inline_text": "Test."}
        manifest_path = artifacts_dir / "abs-path-test.yml"
        manifest_path.write_text(yaml.dump(manifest), encoding="utf-8")
        ...
        test_args = [
            ...
            "abs-path-test",
            "--knowledge-path",
            str(knowledge_path.absolute()),  # Absolute path
            "--mock",
        ]
    ...
    def test_main_artifact_extraction_error(self, tmp_path: Path) -> None:
        """Should return 1 on ArtifactExtractionError (lines 1011-1013)."""
        knowledge_path = tmp_path / ".knowledge"
        (knowledge_path / "artifacts").mkdir(parents=True)
    ...
    def test_main_prints_summary(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        """Should print extraction summary on success (lines 1002-1009)."""
        knowledge_path = tmp_path / ".knowledge"
        (knowledge_path / "facts").mkdir(parents=True)
        (knowledge_path / "facts" / "residue").mkdir(parents=True)
        artifacts_dir = knowledge_path / "artifacts"
        ...
        manifest = {"artifact_id": "summary-test", "inline_text": "Test content."}
        manifest_path = artifacts_dir / "summary-test.yml"
        manifest_path.write_text(yaml.dump(manifest), encoding="utf-8")
        ...
        assert "Artifact ID:" in captured.out
        assert "Total Passes:" in captured.out
        assert "Total Facts Extracted:" in captured.out
        assert "Final State Hash:" in captured.out
        assert "Audit Action:" in captured.out
