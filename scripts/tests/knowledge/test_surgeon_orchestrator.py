...
class TestInvokeSubAgent:
    ...
    def test_invoke_sub_agent_timeout_raises_error(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired("cmd", 120)):
            with pytest.raises(SurgeonError, match="timed out"):
                invoke_sub_agent(
                    "fact-surgeon-organizer",
                    {"input": "test"},
                    timeout=120,
    ...
    def test_invoke_sub_agent_not_found_raises_error(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run", side_effect=FileNotFoundError("claude")):
            with pytest.raises(SurgeonError, match="Claude CLI not found"):
                invoke_sub_agent(
                    "fact-surgeon-organizer",
                    {"input": "test"},
                    knowledge_path=tmp_path,
    ...
    def test_invoke_sub_agent_non_zero_exit_raises_error(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run", return_value=mock_result):
            with pytest.raises(SurgeonError, match="non-zero exit code"):
                invoke_sub_agent(
                    "fact-surgeon-organizer",
                    {"input": "test"},
                    knowledge_path=tmp_path,
    ...
    def test_invoke_sub_agent_empty_output_raises_error(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run", return_value=mock_result):
            with pytest.raises(SurgeonError, match="empty output"):
                invoke_sub_agent(
                    "fact-surgeon-organizer",
                    {"input": "test"},
                    knowledge_path=tmp_path,
    ...
    def test_invoke_sub_agent_invalid_json_raises_error(self, tmp_path: Path) -> None:
        ...
        with patch("subprocess.run", return_value=mock_result):
            with pytest.raises(SurgeonError, match="No JSON found"):
                invoke_sub_agent(
                    "fact-surgeon-organizer",
                    {"input": "test"},
                    knowledge_path=tmp_path,
