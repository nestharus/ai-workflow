...
class TestExtractIdsAndObjects:
    ...
    def test_extracts_from_nested_dict(self) -> None:
        ...
        result, _edges = extract_ids_and_objects(data)  # type: ignore[arg-type]
...
class TestAssignRole:
    ...
    def test_default_returns_constraint(self) -> None:
        ...
        # Pass empty registry_cache to avoid loading real registry which may have artifact roots
        assert (
            _assign_role("text", "scalar-str", "text", None, None, registry_cache=[]).role
            == "constraint"
        )
        assert (
            _assign_role(
                "status_code", "scalar-num", "raises[0].status_code", None, None, registry_cache=[]
            ).role
            == "constraint"
        )
        assert (
...
class TestComputeGroupKey:
    ...
    def test_nested_list_index_extraction(self) -> None:
        ...
        # Without discriminator, should strip index for container matching
        # but return full scope_path
...
class TestArtifactRootDetection:
    def setup_method(self) -> None:
        """Clear the artifact registry cache before each test."""
        _clear_artifact_registry_cache()
    ...
    def test_load_artifact_registry_valid_file(self, fs: FakeFilesystem) -> None:
        ...
        _clear_artifact_registry_cache()  # Clear again since missing_file test sets cache
        fs.create_dir("/fake/.knowledge/artifacts")
        registry_content = """
kinds:
  - kind_id: test/kind
...
class TestGetModalityAndExtractionModeFromRegistry:
    ...
    def test_handles_invalid_modality_value(self) -> None:
        ...
        modality, _extraction_mode = _get_modality_and_extraction_mode_from_registry(
            "test/kind",
            registry_cache=registry,
        )
        assert modality == "text"
    ...
    def test_handles_invalid_extraction_mode_value(self) -> None:
        ...
        _modality, extraction_mode = _get_modality_and_extraction_mode_from_registry(
            "test/kind",
            registry_cache=registry,
        )
        assert extraction_mode == "full"
...
class TestIndexElements:
    def test_indexes_single_element(self) -> None:
        ...
        result = _index_elements(data)  # type: ignore[arg-type]
...
class TestComputeElementContentHash:
    ...
    def test_hash_includes_all_fields(self) -> None:
        ...
        sliced_objects, _edges = extract_ids_and_objects(data)  # type: ignore[arg-type]
