"""Tests for scripts.coderabbit_review module."""
...
from scripts.coderabbit_review import (
...
class TestCoderabbitNotFoundError:
    """Tests for CoderabbitNotFoundError exception."""
...
class TestRunCoderabbit:
    ...
    def test_raises_when_coderabbit_not_found(self, fs: FakeFilesystem) -> None:
        ...
        with patch("shutil.which", return_value=None):
            with pytest.raises(CoderabbitNotFoundError):
    ...
    def test_creates_output_directory(self, fs: FakeFilesystem) -> None:
        ...
        with patch("shutil.which", return_value="/usr/bin/coderabbit"):
            with patch("scripts.coderabbit_review.run_command_with_tee", return_value=0):
    ...
    def test_calls_coderabbit_with_prompt_only(self, fs: FakeFilesystem) -> None:
        ...
        with patch("shutil.which", return_value="/usr/bin/coderabbit"):
            with patch("scripts.coderabbit_review.run_command_with_tee") as mock_tee:
    ...
    def test_raises_system_exit_on_failure(self, fs: FakeFilesystem) -> None:
        ...
        with patch("shutil.which", return_value="/usr/bin/coderabbit"):
            with patch("scripts.coderabbit_review.run_command_with_tee", return_value=1):
