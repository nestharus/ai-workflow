"""Tests for scripts.dev.markdown.convert_docs_architecture_repository_structure module."""
...
class TestMain:
    ...
    def test_main_calls_converter_with_correct_paths(self) -> None:
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_architecture_repository_structure.MarkdownConverter"
        ) as mock_converter_class:
            mock_converter = MagicMock()
            mock_converter.run.return_value = mock_doc
            mock_converter_class.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_architecture_repository_structure import (
                main,
            )
            ...
            assert "repository-structure.md" in call_args[0]
            assert "repository-structure.yml" in call_args[1]
