"""Tests for scripts.dev.markdown.convert_docs_processes_orchestrator_invocation module."""
...
class TestMain:
    ...
    def test_main_with_mocked_converter(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Orchestrator Invocation"
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_processes_orchestrator_invocation.MarkdownConverter"
        ) as MockConverter:
            ...
            assert "Title: Orchestrator Invocation" in captured.out
    ...
    def test_main_creates_correct_paths(self) -> None:
        ...
        with patch(
            ...
            assert "orchestrator-invocation.md" in source_path
            assert "orchestrator-invocation.yml" in target_path
    ...
    def test_main_prints_all_info(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Orchestrator Invocation Doc"
        ...
        with patch(
            ...
            assert "Converted:" in captured.out
            assert "Output:" in captured.out
            assert "Title: Orchestrator Invocation Doc" in captured.out
    ...
    def test_main_path_resolution(self) -> None:
        ...
        with patch(
            ...
            assert source_path.endswith("orchestrator-invocation.md")
            assert "processes" in source_path
            assert target_path.endswith("orchestrator-invocation.yml")
