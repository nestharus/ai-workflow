"""Tests for scripts.dev.markdown.convert_docs_processes_command_patterns module."""
...
class TestMain:
    ...
    def test_main_with_mocked_converter(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Command Patterns"
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_processes_command_patterns.MarkdownConverter"
        ) as MockConverter:
            MockConverter.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_processes_command_patterns import main
            ...
            assert "Title: Command Patterns" in captured.out
    ...
    def test_main_creates_correct_paths(self) -> None:
        ...
        with patch(
            ...
            assert "command-patterns.md" in source_path
            assert "command-patterns.yml" in target_path
    ...
    def test_main_prints_all_info(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Command Patterns Guide"
        mock_doc.sections = [MagicMock() for _ in range(4)]
        ...
        with patch(
            ...
            assert "Converted:" in captured.out
            assert "Output:" in captured.out
            assert "Title: Command Patterns Guide" in captured.out
            assert "Sections: 4" in captured.out
    ...
    def test_main_path_resolution(self) -> None:
        ...
        with patch(
            ...
            assert source_path.endswith("command-patterns.md")
            assert "processes" in source_path
            assert target_path.endswith("command-patterns.yml")
