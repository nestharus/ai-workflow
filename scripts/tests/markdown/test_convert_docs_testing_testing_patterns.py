"""Tests for scripts.dev.markdown.convert_docs_testing_testing_patterns module."""
...
class TestMain:
    ...
    def test_main_with_mocked_converter(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Testing Patterns"
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_testing_testing_patterns.MarkdownConverter"
        ) as MockConverter:
            MockConverter.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_testing_testing_patterns import main
            ...
            assert "Title: Testing Patterns" in captured.out
    ...
    def test_main_creates_correct_paths(self) -> None:
        ...
        with patch(
            ...
            assert "testing-patterns.md" in source_path
            assert "testing-patterns.yml" in target_path
    ...
    def test_main_prints_all_info(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Testing Patterns Guide"
        mock_doc.sections = [MagicMock() for _ in range(8)]
        ...
        with patch(
            ...
            assert "Converted:" in captured.out
            assert "Output:" in captured.out
            assert "Title: Testing Patterns Guide" in captured.out
            assert "Sections: 8" in captured.out
    ...
    def test_main_path_resolution(self) -> None:
        ...
        with patch(
            ...
            assert source_path.endswith("testing-patterns.md")
            assert "testing" in source_path
            assert target_path.endswith("testing-patterns.yml")
