"""Tests for scripts.dev.markdown.convert_docs_processes_inter_domain_handoffs module."""
...
class TestMain:
    ...
    def test_main_with_mocked_converter(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Inter-Domain Handoffs"
        ...
        with patch(
            "scripts.dev.markdown.convert_docs_processes_inter_domain_handoffs.MarkdownConverter"
        ) as MockConverter:
            MockConverter.return_value = mock_converter

            from scripts.dev.markdown.convert_docs_processes_inter_domain_handoffs import main
            ...
            assert "Title: Inter-Domain Handoffs" in captured.out
    ...
    def test_main_creates_correct_paths(self) -> None:
        ...
        with patch(
            ...
            assert "inter-domain-handoffs.md" in source_path
            assert "inter-domain-handoffs.yml" in target_path
    ...
    def test_main_prints_all_info(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        mock_doc = MagicMock()
        mock_doc.title = "Inter-Domain Handoffs Guide"
        ...
        with patch(
            ...
            assert "Converted:" in captured.out
            assert "Output:" in captured.out
            assert "Title: Inter-Domain Handoffs Guide" in captured.out
    ...
    def test_main_path_resolution(self) -> None:
        ...
        with patch(
            ...
            assert source_path.endswith("inter-domain-handoffs.md")
            assert "processes" in source_path
            assert target_path.endswith("inter-domain-handoffs.yml")
