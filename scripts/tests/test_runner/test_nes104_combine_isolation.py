"""NES-104 Validation Tests for Requirements 5-6.

Requirement 5: Combine Coverage for Redundant Test Detection
- Combine command executes successfully
- Combined file contains data from all tiers
- CRITICAL: Per-test contexts are preserved (suspected broken feature)
- Combine doesn't corrupt individual tier files (--keep flag works)
- Combine with single tier file still works

Requirement 6: Preserve Per-Tier Isolation
- Tier validation reads only tier-specific data (uses self.coverage_result, not combined)
- Function coverage written per-tier to cc_function_coverage table
- Validation doesn't use combined file for per-tier thresholds
- Deleting combined file doesn't affect tier validation

These tests create minimal SQLite coverage databases with context tables to verify
that contexts survive the combine operation - this is the critical feature for
...
class TestCombineCommand:
    """Tests for coverage combine command execution."""
...
class TestCombinedDataContent:
    """Tests verifying combined file contains data from all tiers."""
...
class TestContextPreservation:
    """CRITICAL: Tests for per-test context preservation during combine.

    This is the suspected broken feature. The redundant test detector relies on
    per-test contexts being preserved after combine. If contexts are lost or
    merged incorrectly, the detector will fail.
    """
...
class TestCombineKeepFlag:
    """Tests verifying --keep flag preserves original tier files."""
...
class TestTierIsolation:
    """Tests verifying per-tier isolation during validation.

    The key invariant is that tier validation (LineBranchTestStrategy.validate())
    reads from self.coverage_result which is built from the tier-specific coverage
    file, NOT from the combined file.
    """
...
class TestFunctionCoveragePerTier:
    """Tests verifying function coverage is written per-tier to database."""
...
class TestTierThresholdIsolation:
    """Tests verifying per-tier thresholds are evaluated independently."""
...
class TestDeletingCombinedFileDoesNotAffectValidation:
    """Integration test verifying combined file is not needed for validation."""
...
# =============================================================================
# NES-104 ACTUAL IMPLEMENTATION VERIFICATION
#
# These tests verify the ACTUAL behavior of test_coverage.py and test_strategies.py
# rather than just testing mocked/expected behavior. They capture the real commands
# constructed by the implementation to ensure:
#
# 1. --context=test flag is passed to pytest (required for per-test contexts)
# 2. Combine command includes --keep flag
# 3. Tier validation uses tier-specific coverage file, NOT combined
#
# Without --context=test, contexts won't exist in tier files, and combine
# will produce a database without contexts - breaking redundant test detection.
# =============================================================================


class TestCovContextFlagInPytestCommand:
    """CRITICAL: Tests verifying --context=test flag is passed in pytest commands.

    The --context=test flag is REQUIRED for per-test contexts to be recorded.
    Without this flag:
    - Coverage data has no per-test context information
    - Combined database will have no contexts
    - Redundant test detection will fail (all tests appear redundant)

    This is the suspected ROOT CAUSE of broken redundant test detection.
    """
...
class TestCombineCommandConstruction:
    """Tests verifying the combine command in main() is constructed correctly.

    The combine command MUST include --keep flag to preserve tier-specific files
    after combining. Without --keep, tier files are deleted, breaking per-tier
    validation in subsequent runs.
    """
...
class TestTierValidationUsesCorrectFile:
    """Tests verifying tier validation uses tier-specific file, NOT combined.

    The validation logic must read from self.coverage_result which is built
    from the tier-specific coverage file, not from the combined file.
    """
...
class TestContextPreservationEndToEnd:
    """End-to-end tests verifying contexts survive the full pipeline.

    These tests verify that when --context=test is used:
    1. Tier files contain per-test contexts
    2. Combined file preserves all contexts from all tiers
    3. Redundant test detector can analyze contexts correctly
    """
