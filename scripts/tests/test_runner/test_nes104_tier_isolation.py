"""Validation tests for NES-104 Requirements 1-2: Tier Isolation.

This module contains comprehensive tests validating NES-104's tier isolation implementation:

Requirement 1: Per-Tier Coverage Files
- Unit tier creates `.coverage/data_unit`
- Component tier creates `.coverage/data_component`
- Multiple tiers create distinct files (different hashes)
- Old shared data file not used
- Running tier in isolation doesn't create other tiers' files

Requirement 2: Remove --cov-append
- No `--cov-append` flag in pytest command construction
- No `first_tier` parameter exists in function signatures
- Each tier starts with clean database (metrics identical run alone vs combined)

Test Design:
- Uses pytest fixtures and tmp_path for isolation
- Mocks _run_command to avoid running actual pytest
- Creates minimal SQLite coverage databases for testing
- Focuses on UNIT tests verifying behavior without running actual pytest
"""
...
class TestPerTierCoverageFileCreation:
    """Tests verifying each tier creates distinct coverage data files.

    Requirement 1 specifies that each tier writes to an isolated coverage file:
    - `.coverage/data_unit` for unit tier
    - `.coverage/data_component` for component tier
    - `.coverage/data_integration` for integration tier
    - `.coverage/data_scripts` for scripts tier
    """
...
class TestMultipleTiersCreateDistinctFiles:
    """Tests verifying multiple tiers create distinct coverage files with different content."""
...
class TestOldSharedDataFileNotUsed:
    """Tests verifying the old shared `.coverage/data` file is not used."""
...
class TestTierIsolationDoesNotCreateOtherTierFiles:
    """Tests verifying running one tier doesn't create other tiers' coverage files."""
...
class TestNoCovAppendFlag:
    """Tests verifying --cov-append flag is NOT used in pytest command construction."""
...
class TestNoFirstTierParameter:
    """Tests verifying no first_tier parameter exists in function signatures."""
...
class TestCleanDatabasePerTier:
    """Tests verifying each tier starts with clean coverage data (not appended)."""
    ...
    def test_tier_starts_fresh_with_isolated_coverage_file(self, tmp_path: Path) -> None:
        ...
        # Verify no --cov-append in command
        # (Cannot verify file overwrite without actually running pytest)
...
class TestCoverageIsolationContractWithFactory:
    """Tests verifying the factory respects tier isolation contracts."""
...
class TestEdgeCases:
    """Edge case tests for tier isolation."""
...
class TestNegativeCases:
    """Negative test cases to ensure wrong patterns are rejected."""
...
# =============================================================================
# ACTUAL IMPLEMENTATION BEHAVIOR TESTS (NES-104 Enhancement)
# =============================================================================
# These tests verify that the ACTUAL implementation produces correct results
# by intercepting subprocess calls at the lowest level and verifying what
# WOULD be executed. They do NOT mock command construction logic.
# =============================================================================


class TestActualImplementationCoverageFileEnvironment:
    """Tests verifying actual COVERAGE_FILE env var is set when pytest is invoked.

    These tests capture the actual subprocess call arguments to verify that the
    real implementation correctly sets the COVERAGE_FILE environment variable.
    Unlike the mock-based tests, these tests let the actual strategy code run
    and only intercept at the subprocess boundary.
    """
...
class TestActualImplementationNoCovAppend:
    """Tests verifying actual pytest command does NOT contain --cov-append.

    These tests intercept subprocess.run() to capture the actual command
    that would be executed and verify it does NOT contain --cov-append.
    """
...
class TestActualImplementationCoverageFilePaths:
    """Tests verifying actual coverage file path creation behavior.

    These tests verify that running a tier correctly configures coverage to
    write to the expected file path.
    """
...
class TestActualImplementationCommandStructure:
    """Tests verifying actual pytest command structure.

    These tests verify that the actual command constructed by the implementation
    contains all expected flags and arguments.
    """
...
class TestActualImplementationIntegrationStrategy:
    """Tests verifying actual IntegrationTestStrategy command structure.

    IntegrationTestStrategy should NOT set COVERAGE_FILE since it uses
    usecase coverage, not line/branch coverage.
    """
