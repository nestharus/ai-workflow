...
def test_entity_search_boost():
    ...
    results_with_entity = searcher.search(
        "user profiles", entity_names=["ENT-0001"]
    )
    # Search without entity name
    results_without_entity = searcher.search("user profiles")
...
def test_search_for_ambiguity():
    ...
    top = results[0]
    assert "gateway" in top.paragraph.text.lower() or "timeout" in top.paragraph.text.lower() or "payment" in top.paragraph.text.lower()
