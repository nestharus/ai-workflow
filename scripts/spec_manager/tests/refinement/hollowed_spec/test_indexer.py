...
def test_build_index_from_single_spec():
    ...
    assert "authenticate" in index.global_keyword_index or "authentication" in index.global_keyword_index
...
def test_remove_and_rebuild():
    ...
    for kw, entries in index.global_keyword_index.items():
...
def test_index_persistence_roundtrip(tmp_path: Path):
    ...
    assert set(loaded.global_keyword_index.keys()) == set(
        index.global_keyword_index.keys()
    )

    # Verify entity index equality
    assert set(loaded.global_entity_index.keys()) == set(
        index.global_entity_index.keys()
    )
...
def test_add_spec_replaces_existing():
    ...
    para_count_1 = index.total_paragraphs

    hollowed2 = hollow_out_spec("LIB-0001", SPEC_B)
    index.add_spec(hollowed2)
    para_count_2 = index.total_paragraphs
