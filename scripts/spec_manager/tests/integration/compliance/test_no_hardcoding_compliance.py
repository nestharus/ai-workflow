...
class TestGapDetectorsNoHardcoding:
    ...
    def test_prose_detector_uses_structural_without_llm(self, gap_detection_input: str) -> None:
    ...
    def test_no_forbidden_patterns_in_gaps_py(self) -> None:
        ...
        keyword_violations = [v for v in violations if v.pattern_type == "keyword_inference"]
        assert len(keyword_violations) == 0, (
            f"Found violations: {[v.message for v in keyword_violations]}"
        )
...
class TestHeuristicFallbacksNonAuthoritative:
    ...
    def test_heuristic_resolution_confidence_below_threshold(self) -> None:
        ...
        _, confidence, rationale = strategy._resolve_with_heuristic("the algorithm", units, 1)
...
class TestForbiddenPatternAssertions:
    ...
    def test_no_modal_verb_regex_in_detection(self) -> None:
        ...
        for _pattern in forbidden_patterns:
