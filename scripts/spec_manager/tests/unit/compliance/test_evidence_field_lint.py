...
class TestScanEvidenceFields:
    ...
    def test_spec_snapshot_in_evidence_fails(self) -> None:
        ...
        artifact = {
            "file_id": "F0001",
            "spec_ref": "[spec_snapshot/file.md::SEC-F0001-0001]",
        }
        errors, _warnings = scan_evidence_fields(artifact)
        assert len(errors) == 1
        assert "Legacy citation" in errors[0]
    ...
    def test_allowlist_suppression(self) -> None:
        ...
        # With allowlist
        errors, warnings = scan_evidence_fields(artifact, allowlist={"evidence": ["SPECIAL_VALUE"]})
        assert len(errors) == 0
