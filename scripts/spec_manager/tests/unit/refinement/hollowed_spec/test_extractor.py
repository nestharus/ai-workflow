...
def test_paragraph_classification():
    ...
    assert _classify_paragraph("- item 1\n- item 2\n- item 3") == ParagraphKind.BULLET_LIST

    assert _classify_paragraph("```python\ndef foo():\n    pass\n```") == ParagraphKind.CODE_BLOCK

    assert (
        _classify_paragraph("| Name | Value |\n| --- | --- |\n| foo | bar |") == ParagraphKind.TABLE
    )

    assert _classify_paragraph("1. First\n2. Second\n3. Third") == ParagraphKind.BULLET_LIST
...
def test_paragraph_line_numbers():
    ...
    for _para_id, para in result.paragraphs.items():
