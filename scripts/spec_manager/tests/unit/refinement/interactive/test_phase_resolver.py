...
def test_no_ambiguities_passthrough(tmp_path: Path) -> None:
    ...
    with patch.object(pr._detector, "detect_signals", return_value=[]) as mock_detect:
...
def test_detect_resolve_patch_loop(tmp_path: Path) -> None:
    ...
    with patch.object(pr._detector, "detect_signals", side_effect=[[signal], []]):
        result = pr.resolve_phase_output(tmp_path, "sectionization", "input text")
...
def test_max_iterations_respected(tmp_path: Path) -> None:
    ...
    with patch.object(pr._detector, "detect_signals", return_value=[signal]) as mock_detect:
...
def test_library_id_in_work_context(tmp_path: Path) -> None:
    ...
    with patch.object(pr._detector, "detect_signals", return_value=[]) as mock_detect:
        pr.resolve_phase_output(tmp_path, "spec_building", "text", library_id="LIB-001")
