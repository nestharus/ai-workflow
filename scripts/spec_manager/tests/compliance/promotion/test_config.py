...
class TestGateCheckResult:
    ...
    def test_to_dict_with_findings(self) -> None:
        result = GateCheckResult(
            ...
            score=0.0,
            findings=[
                {"file_path": "foo.py", "line": 10, "name": "bar", "stub_type": "pass"}
            ],
...
class TestPromotionReport:
    def test_to_dict(self) -> None:
        result1 = GateCheckResult(
            gate_id="gate_a", passed=True, mode="required"
        )
        result2 = GateCheckResult(
            gate_id="gate_b", passed=False, mode="advisory"
        )
    ...
    def test_save(self) -> None:
        result = GateCheckResult(
            gate_id="save_test", passed=True, mode="required"
        )
        report = PromotionReport(
            passed=True,
            gate_results=[result],
