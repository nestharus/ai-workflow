...
class PddLifecycle:
    ...
    def build(self) -> dict[str, Any]:
        ...
        # Run the pipeline in loop mode (per-slice PromotionLoop) or
        # legacy sequential mode.
        pipeline_result = self.orchestrator.run()
        outputs["pipeline"] = pipeline_result

        # Refine library specs (ambiguity detection + resolution)
        ...
        # Generate overview for human review
        overview_result = self._generate_overview()
        outputs["overview"] = overview_result

        # Setup worktrees for parallel implementation if manager available
        if self.worktree_manager:
