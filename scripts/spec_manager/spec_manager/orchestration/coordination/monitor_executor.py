...
class ConditionChecker:
    ...
    def _check_constraint_present(self, cond: ConstraintPresentCondition) -> bool:
        """Check if a constraint file exists on disk."""
        if not cond.constraint_key:
            return False
        constraint_dir = self._workspace_root / cond.constraint_dir
        # Look for a file whose name contains the constraint key
        if not constraint_dir.exists():
            return False
        for p in constraint_dir.iterdir():
            if cond.constraint_key in p.name:
                return True
        return False
    ...
    def _check_slice_merged(self, cond: SliceMergedCondition) -> bool:
        ...
        marker = (
            ...
            / "slices"
            / cond.provider_slice_id
            / f"merged_{cond.layer}"
        )
