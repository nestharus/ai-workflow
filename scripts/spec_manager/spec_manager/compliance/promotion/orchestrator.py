...
class LayerPromotionGate:
    ...
    def run_all_checks(self) -> PromotionReport:
        ...
        if gate_spec.enabled:
            if self._pin_registry is not None:
                ...
                results.append(self._skip_gate(
                    GateId.PIN_COVERAGE, gate_spec,
        ...
        if gate_spec.enabled:
            if self._pin_registry is not None and pin_coverage_report is not None:
                results.append(check_introduced_algorithm_specs(
                    pin_coverage_report, architectural_files, gate_spec
                ))
            else:
                results.append(self._skip_gate(
                    GateId.INTRODUCED_ALGORITHM_SPECS, gate_spec,
                    "PinFunctionRegistry not provided"
                ))
        ...
        if gate_spec.enabled:
            if self._pin_registry is not None:
                ...
                results.append(check_provenance_complete(
                    self._pin_registry, provenance_registry, gate_spec
                ))
            else:
                results.append(self._skip_gate(
                    GateId.PROVENANCE_COMPLETE, gate_spec,
                    "PinFunctionRegistry not provided"
                ))
        ...
        if gate_spec.enabled:
            if self._pin_registry is not None:
                results.append(check_no_inlined_atom_logic(
                    self._pin_registry, architectural_files,
                    algorithmic_files, gate_spec
                ))
            else:
                results.append(self._skip_gate(
                    GateId.NO_INLINED_ATOM_LOGIC, gate_spec,
                    "PinFunctionRegistry not provided"
                ))
        ...
        if gate_spec.enabled:
            if self._pin_registry is not None:
                results.append(check_function_recomposition(
                    self._pin_registry, architectural_files, gate_spec
                ))
            else:
                results.append(self._skip_gate(
                    GateId.FUNCTION_RECOMPOSITION, gate_spec,
                    "PinFunctionRegistry not provided"
                ))
        ...
        if gate_spec.enabled:
            if self._pin_registry is not None:
                test_roots = [
                    self._project_root / r
                    for r in gate_spec.params.get(
                        "test_roots", ["tests/"]
                    )
                ]
                baseline_path = self._project_root / ".spec" / "test_pin_baselines.json"
                results.append(check_test_pin_alignment_gate(
                    self._pin_registry, test_roots, baseline_path, gate_spec
                ))
            else:
                results.append(self._skip_gate(
                    GateId.TEST_PIN_ALIGNMENT, gate_spec,
                    "PinFunctionRegistry not provided"
                ))
        ...
        if gate_spec.enabled:
            if self._evidence_index is not None:
                ...
                results.append(check_entity_coverage(
                ...
                results.append(self._skip_gate(
                    GateId.ENTITY_COVERAGE, gate_spec,
    ...
    def run_single_check(self, gate_id: GateId) -> GateCheckResult:
        ...
        runners: dict[GateId, Any] = {
            ...
            GateId.NO_STUB_FUNCTIONS: lambda: check_no_stub_functions(
                algorithmic_files, gate_spec
            ),
            ...
            GateId.PIN_COVERAGE: lambda: check_pin_coverage(
                self._pin_registry, architectural_files, gate_spec
            ) if self._pin_registry else self._skip_gate(
                GateId.PIN_COVERAGE, gate_spec,
                "PinFunctionRegistry not provided"
            ),
    ...
    def _run_introduction_check(
        ...
        pin_coverage = build_pin_coverage_report(
            self._pin_registry, architectural_files
        )
        return check_introduced_algorithm_specs(
            pin_coverage, architectural_files, gate_spec
        )
    ...
    def _run_test_pin_alignment(self, gate_spec: GateSpec) -> GateCheckResult:
        ...
        test_roots = [
            self._project_root / r
            for r in gate_spec.params.get("test_roots", ["tests/"])
        ]
        baseline_path = self._project_root / ".spec" / "test_pin_baselines.json"
