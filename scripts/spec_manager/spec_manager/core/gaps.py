...
class StructuralHealthDetector:
    ...
    def detect(self, units: list, libraries: dict[str, list[str]] | None = None) -> list[GapEvidence]:
        """
    ...
    def _detect_coupling_issues(self, units: list, libraries: dict[str, list[str]] | None = None) -> list[GapEvidence]:
    ...
    def _detect_cohesion_issues(self, units: list, libraries: dict[str, list[str]] | None = None) -> list[GapEvidence]:
    ...
    def _detect_unsatisfied_invariants(self, units: list) -> list[GapEvidence]:
        ...
        invariants = {getattr(u, 'id', '') for u in units if str(getattr(u, 'unit_type', '')).lower() in ('invariant', 'p#i#')}
        referenced = set()
...
class EvidenceExtractor:
    ...
    def extract_from_content(
        ...
        if all_detectors or 'format' in (detectors or []):
            evidence.extend(normalize_to_evidence(
                self.format_detector.detect(content, file_path)
            ))
        ...
        if all_detectors or 'sequence' in (detectors or []):
            evidence.extend(normalize_to_evidence(
                self.sequence_analyzer.detect(content, file_path)
            ))

        if all_detectors or 'uncertainty' in (detectors or []):
            evidence.extend(normalize_to_evidence(
                self.uncertainty_detector.detect(content, file_path)
            ))
    ...
    def extract_from_units(
        self,
        units: list,
        libraries: dict[str, list[str]] | None = None
    ) -> list[GapEvidence]:
...
class UnifiedGapDetector:
    ...
    def collect_evidence(
        ...
        # Detect in plan.md
        if plan_content:
            evidence.extend(self.evidence_extractor.extract_from_content(
        ...
        if libraries_dir.exists() and plan_content:
            for lib_file in libraries_dir.glob("*.md"):
                ...
                evidence.extend(self.evidence_extractor.extract_from_comparison(
    ...
    def format_gaps_md(
        ...
        if by_patch:
            for patch_id in sorted(by_patch.keys()):
                ...
                for gap in patch_gaps:
                    icon = {'error': '[ERROR]', 'warning': '[WARN]', 'info': '[INFO]'}.get(gap.severity.value, '-')
        ...
        proof_gaps = [g for g in gaps if 'proof' in g.id.lower() or 'claim' in g.id.lower() or
                      any('proof' in str(e.message).lower() or e.detector == 'proof_chain' for e in g.evidence)]
        if proof_gaps:
        ...
        undefined_gaps = [g for g in gaps if 'undefined' in g.id.lower() or 'function' in g.id.lower() or
                         any(e.detector == 'undefined_function' for e in g.evidence)]
        if undefined_gaps:
        ...
        structural_gaps = [g for g in no_patch if 'structural' in g.id.lower() or 'coupling' in g.id.lower() or
                          ...
                          any(e.detector == 'structural_health' for e in g.evidence)]
...
def _detect_undefined_references(
    content: str, registry
) -> list[dict[str, Any]]:
    ...
    for id_value, section in result.sections.items():
        for match in todo_pattern.finditer(section.body):
            ...
            gaps.append({
                ...
                "source_id": id_value,
                "marker": marker,
                "description": f"{marker} in {id_value}: {context}" if context else f"{marker} marker in {id_value}",
                "severity": "warning",
            })
...
def _detect_invalid_libraries(libraries_dir: Path) -> list[dict[str, Any]]:
    ...
    for lib_file in libraries_dir.glob("*.md"):
        ...
        type_based_names = ["data", "algorithms", "goals", "invariants", "helpers", "utils", "common", "misc"]
        if lib_name.lower() in type_based_names:
            ...
            if not existing:
                gaps.append({
                    ...
                    "severity": "warning",
                    "suggestion": f"Rename to describe the subsystem/domain capability, not the content type",
                })

        # Check if library contains invariants/goals (should be at root level)
        extractor = SectionExtractor()
        result = extractor.extract(content)
        invariant_ids = [id_val for id_val in result.sections.keys() if re.match(r"^[GI]\d+", id_val)]
        if invariant_ids:
            gaps.append({
...
def _detect_cohesion_issues(
    content: str, registry, libraries_dir: Path
) -> list[dict[str, Any]]:
    """
    Detect subsystem cohesion issues.
    """
    ...
    for id_a, id_b in cross_lib_clusters:
        ...
        gaps.append({
            ...
            "severity": "info",
            "suggestion": f"Consider moving to same library if they represent a cohesive subsystem",
        })
...
def format_gaps_md(gaps: list[dict[str, Any]]) -> str:
    ...
    for gap_type in sorted(by_type.keys()):
        ...
        for gap in type_gaps:
            ...
            if "is_legacy_goal" in gap and gap["is_legacy_goal"]:
                lines.append(f"  - Note: Consider migrating G# to I# (invariant)")
            lines.append("")
