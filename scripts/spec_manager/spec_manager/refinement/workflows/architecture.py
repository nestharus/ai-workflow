...
def _build_brief_extraction_prompt(lib_id: str, charter: str, spec: str) -> str:
    lines = [
        ...
        "{",
        '  "lib_id": "LIB-0001",',
        '  "intent": "Handle inbound request intake.",',
        '  "boundaries": "Covers validation and routing only.",',
        '  "dependencies": ["External Auth Service"],',
        '  "constraints": [',
        "    {",
        '      "type": "Security",',
        '      "description": "Must enforce OAuth2 tokens.",',
        '      "citation": "[LIB-0001::spec.md::SECURITY]"',
        "    }",
        "  ],",
        '  "interfaces": [',
        "    {",
        '      "type": "HTTP",',
        '      "description": "POST /requests endpoint.",',
        '      "citation": "[LIB-0001::spec.md::API]"',
        "    }",
        "  ]",
        "}",
    ]
...
def _build_architecture_proposal_prompt(
    ...
    lines = [
        ...
        "  - [LIB-####::charter.md]",
        "  - [LIB-####::spec.md::SECTION] (SECTION from allowlist below)",
    ...
    lines.extend(
        [
            ...
            '    "description": "Layered architecture separating API, domain, and data access.",',
            '    "components": [',
            "      {",
            '        "name": "API Layer",',
            '        "responsibilities": ["Handle request routing", "Validate inputs"]',
            "      }",
            "    ],",
            '    "communication": "Synchronous HTTP between layers.",',
            '    "deployment": "Single service with modular packages.",',
            '    "citations": ["[LIB-0001::charter.md]", "[LIB-0002::spec.md::API]"],',
            '    "tradeoffs": {',
            '      "advantages": ["Clear separation of concerns"],',
            '      "disadvantages": ["Potential latency between layers"]',
            "    }",
            "  }",
            "]",
        ]
...
def _build_architecture_selection_prompt(
    ...
    lines.extend(
        [
            ...
            '  "rationale": "Balances modularity with operational simplicity. '
            '[LIB-0001::charter.md] [LIB-0002::spec.md::TRADEOFFS]",',
            '  "rejected_architectures": [',
            "    {",
            '      "arch_id": "arch_002",',
            '      "rationale": "Adds deployment complexity without clear benefit."',
            "    }",
            "  ],",
            '  "implementation_risks": ["Component boundaries may require refactoring."],',
            '  "evolution_notes": ["Revisit for scalability after phase 2."]',
            "}",
        ]
    )
    return "\n".join(lines).strip() + "\n"
...
def _build_library_mapping_prompt(
    ...
    lines = [
        ...
        "- Citations MUST use library pointers only: [LIB-####::charter.md] or "
        "[LIB-####::spec.md::SECTION]",
        ...
        '  "lib_id": "LIB-0001",',
        '  "component": "API Layer",',
        '  "rationale": "Owns request routing responsibilities.",',
        '  "citations": ["[LIB-0001::charter.md]"],',
        '  "cross_component_dependencies": [',
        "    {",
        '      "target_component": "Data Layer",',
        '      "reason": "Needs persistence access for request state.",',
        '      "citation": "[LIB-0001::spec.md::DATA]"',
        "    }",
        "  ]",
        "}",
    ]
