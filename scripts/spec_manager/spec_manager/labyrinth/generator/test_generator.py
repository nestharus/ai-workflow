...
class TestGenerator:
    ...
    def generate_test_suite(
        ...
        lines.append(
            "2. Integration Completeness: EventLog verification for integration points and chains"
        )
        lines.append('"""')
        lines.append("")
        ...
        for i, test_case in enumerate(rule_tests):
            ...
            lines.append("    )")
            lines.append("    results = pipeline.process_sync(record)")

            if should_fire:
                lines.append(
                    f'    rule_results = [r for r in results if r.source_rule_id == "{rule_id}"]'
                )
                lines.append(
                    f'    assert len(rule_results) > 0, "Rule {rule_id} should have fired"'
                )
                ...
                lines.append(
                    f'    assert len(rule_results) == 0, "Rule {rule_id} should NOT have fired"'
                )
        ...
        lines.append(
            '    """All integration points should have their required rules registered."""'
        )
