...
Usage:
    uv run grep-yml-ids --id <element-id> [--path <directory>] [--exact] [--output json|table]
...
Example:
    uv run grep-yml-ids --id url.prefix --path docs/development/project
    uv run grep-yml-ids --id validation --output json
    uv run grep-yml-ids --id apperror.canonical-envelope --exact
"""
