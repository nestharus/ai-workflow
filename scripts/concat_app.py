...
def build_parser() -> argparse.ArgumentParser:
    """Construct the CLI argument parser for the app concatenation script."""
    parser = argparse.ArgumentParser(
        description=(
            "Concatenate all files under app/ into a single file with headings "
...
def _iter_app_files() -> list[Path]:
    """Return sorted files within app/ (recursive)."""
    if not APP_DIR.is_dir():
        msg = f"App directory not found at {APP_DIR}"
        raise FileNotFoundError(msg)

    files = [
        path for path in APP_DIR.rglob("*") if path.is_file() and "__pycache__" not in path.parts
    ]
    return sorted(
        files,
