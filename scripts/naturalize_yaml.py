...
def process_yaml_file(file_path: Path) -> tuple[bool, dict]:
    ...
    while i < len(lines):
        ...
        type_match = re.match(r'^(\s*)type:\s*(text|rule|step|note|example|code|definition|anti_pattern|pattern)\s*$', line)
        if type_match:
        ...
        if item_start_match:
            ...
            if i + 1 < len(lines):
                next_line = lines[i + 1]
                if not re.match(r'^\s*id:\s*\S+', next_line):
                    # No ID found, add one
        ...
        try:
            ...
            if changed:
                ...
                for type_val, count in stats["text_renamed"].items():
                    total_stats["text_renamed"][type_val] = total_stats["text_renamed"].get(type_val, 0) + count
