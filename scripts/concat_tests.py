...
def build_parser() -> argparse.ArgumentParser:
    """Construct the CLI argument parser for the tests concatenation script."""
    parser = argparse.ArgumentParser(
        description=(
            "Concatenate all files under tests/ into a single file with headings "
            "using paths relative to the repository root."
        ),
    )
    parser.add_argument(
        "--output",
...
def _iter_tests_files() -> list[Path]:
    """Return sorted files within tests/ (recursive, excluding __pycache__)."""
    if not TESTS_DIR.is_dir():
        msg = f"Tests directory not found at {TESTS_DIR}"
        raise FileNotFoundError(msg)

    files = [
        path for path in TESTS_DIR.rglob("*") if path.is_file() and "__pycache__" not in path.parts
    ]
    return sorted(
        files,
