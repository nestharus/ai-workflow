...
2. Rename 'text:' to semantic field based on type
   (rule:, note:, anti_pattern:, pattern:, reference:)
3. Rename 'items:' to semantic collection name
   (rules:, notes:, anti_patterns:, patterns:, references:)
...
def naturalize_yaml_file(file_path: Path) -> dict[str, int]:
    ...
    for section_match in re.finditer(
        ...
        items_match = re.search(
            rf"{indent}  "
            r"(items|rules|notes|patterns|anti_patterns|references):\n"
            rf"((?:{indent}    .*\n?)*)",
            section_text,
        )
        if not items_match:
        ...
        if len(type_counts) == 1:
            ...
            single_type = next(iter(type_counts.keys()))
            semantic_collection = {
                "rule": "rules",
                "note": "notes",
                "pattern": "patterns",
    ...
    def replace_type_text_with_title(match: re.Match[str]) -> str:
