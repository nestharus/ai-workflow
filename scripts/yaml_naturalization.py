...
def naturalize_yaml_file(file_path: Path) -> dict[str, int]:
    ...
    for section_match in re.finditer(r'( +)- id: [^\n]+\n(?:.*?(?=\n\1- id:|\n(?!\1  )|\Z))', content, re.DOTALL):
        section_text = section_match.group(0)
        indent = section_match.group(1)

        # Find items: block in this section
        items_match = re.search(rf'{indent}  (items|rules|notes|patterns|anti_patterns|references):\n((?:{indent}    .*\n?)*)', section_text)
        if not items_match:
            continue
        ...
        if len(type_counts) == 1:
            ...
            if semantic_collection != 'items':
                ...
                content = content.replace(section_text, section_text.replace(old_items_line, new_items_line, 1))
...
def main():
    ...
    for yaml_file in sorted(yaml_files):
        ...
        if 'plans' in rel_path or 'MODULE-DEFINITIONS' in rel_path or 'domain-definitions' in rel_path:
            continue

        stats = naturalize_yaml_file(yaml_file)
        if sum(stats.values()) > 0:
            results.append((rel_path, stats))
            print(f"{rel_path}: removed {stats['types_removed']} types, "
