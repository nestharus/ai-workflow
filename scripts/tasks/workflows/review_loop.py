...
class ReviewIteration:
    ...
    Attributes:
        ...
        revision_status: Status from plan-reviser (REVISED/UNCHANGED/BLOCKED), None if review was APPROVED or BLOCKED.
        addressed_issues: List of issue IDs addressed by the reviser, None if no revision occurred.
        revision_summary: Summary of changes made, None if no revision occurred.
        error_reason: Diagnostic message when review or revision is BLOCKED due to parse/validation errors.
    """
...
def _parse_yaml_after_marker(output: str, marker: str) -> dict:
    ...
    try:
        ...
        if not isinstance(parsed, dict):
            raise ValueError(f"Expected YAML dict after '{marker}', got {type(parsed)}")
        return parsed
...
original_plan: {plan_path}"""
            ...
            revision_status, addressed_issues, revision_summary, revision_error = _parse_revision_output(
